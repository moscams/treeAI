...
5. **底部工具栏不再占地方**（`ChatNode.tsx` / `SystemNode.tsx`）
   - 整条底栏删掉：「+」改成**绝对定位悬浮在节点右下角**（外层 wrapper 不能
     `overflow:hidden`，否则被裁）——放右下而不是底边中央，是因为底部中央正是
     连线起点（source handle），分叉时几条边都从那里出发，浮在中间会撞在一起；
     复制/重试挪进回答右上角的悬停浮层（报错态在占位文案旁保留一个「重试」按钮，
     保证入口还在）。
6. **统计加年份**（`SessionStats.tsx`）：日期格式加 `year: 'numeric'`，并跟随语言切 `zh-CN`/`en-US`。
7. **入/出改成上下箭头**：统计浮层用 `↓ 输入 token` / `↑ 输出 token`；
   节点底部的 `入 X · 出 Y` → `↓ X · ↑ Y`。
8. **侧边栏底部并成一行**（`Sidebar.tsx`）
   - 用户反馈的「占地方」其实指的是「新建会话」下面那排设置/主题按钮（两行两条
     分隔线，白占约 60px）。现在合并成一行：新建按钮 `flex-1`，设置/主题两个图标
     贴在右侧。底部高度 118px → 61px。
